<div align="center">
<img width="1200" height="475" alt="GHBanner" src="https://github.com/user-attachments/assets/0aa67016-6eaf-458a-adb2-6e31a0763ed6" />
</div>

## Run Locally

**Prerequisites:** Bun (recommended) or Node.js

1. Install dependencies:
   `bun install` (or `npm install` / `yarn install` if you prefer)
2. Set the required environment variables in `.env.local` (see the AI & SEO sections below).
3. Run the dev server:
   `bun run dev`

To run a production build locally:
- `bun run build`
- `bun run start`

## SEO & Sitemap

- Set `SITE_URL` (server) and `NEXT_PUBLIC_SITE_URL` (client) in `.env.local` (or `.env.example`) so canonical URLs and Open Graph links reflect your production domain.
- `sitemap.xml` and `robots.txt` are generated by Next via `app/sitemap.ts` and `app/robots.ts`.
- Sitemap is available at `/sitemap.xml`.
- Robots is available at `/robots.txt` and references the sitemap.

## AI recommendations

- Groq responses are cached for `RECOMMENDATION_CACHE_TTL_MS` milliseconds on the server (default 900000ms = 15 minutes) and the dashboard respects the same TTL on the client via `NEXT_PUBLIC_RECOMMENDATION_CACHE_TTL_MS`.
- Once a rate limit hits, the backend enforces `GROQ_COOLDOWN_MS` before making more requests; during the cooldown the dashboard hides the retry button, shows the remaining wait time, and still renders deterministic remediation guidance generated through `generateRecommendations`.
- Flip `FORCE_AI_RECS=true` to skip Groq entirely and always receive the heuristic remediation plan, which is handy when you’re trying to conserve quota or debug without hitting the AI provider.
- Set `GROQ_API_KEY` in your `.env.local` (matching the free tier key) and optionally override `GROQ_MODEL`, `GROQ_TIMEOUT_MS`, or `GROQ_MAX_RETRIES`.
